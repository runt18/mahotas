...
def read_bw(fname, options):
    ...
    print_error("{0} is not a greyscale image (and --convert-to-bw was not specified)".format(fname), not options.no_color)
    sys.exit(1)
...
For example, use --haralick switch to compute Haralick features\n''')
    ...
    if not args.clobber and path.exists(args.output):
        print_error('Output file ({0}) already exists. Refusing to overwrite results without --clobber argument.'.format(args.output))
        sys.exit(2)
    ...
    if args.haralick:
        ...
        colnames.extend(["mean:{0}".format(ell) for ell in hlabels])
        colnames.extend(["ptp:{0}".format(ell) for ell in hlabels])
    if args.lbp:
